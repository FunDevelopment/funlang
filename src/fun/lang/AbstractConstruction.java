...
abstract public class AbstractConstruction extends AbstractNode implements Construction {
    ...
    static ResolvedInstance resolveInstance(Instantiation instance, Context context, boolean shareContext) throws Redirection {
        ...
        if (instance instanceof ResolvedInstance) {
            ...
            FunNode reference = instance.getReference();
    ...
    private Object generateData(Context context, Definition def, FunDebugger debugger) throws Redirection {
        if (debugger == null) {
    ...
    public Object getData(Context context, Definition def) throws Redirection {
        FunDebugger debugger = (context != null ? context.getDebugger() : null);
        if (debugger != null) {
                ...
                if ((cacheability & CACHE_RETRIEVABLE) == CACHE_RETRIEVABLE) {
                    ...
                    if (holder != null && holder.data != null) {
                        ...
                        if (nameNode.numParts() > 1) {
                            ...
                            if (parentHolder != null && parentHolder.data instanceof FunObjectWrapper) {
                                FunObjectWrapper parentObj = (FunObjectWrapper) parentHolder.data;
                ...
                if (data == null) {
                    if (Name.THIS.equals(name)) {
                        ...
                        if (entry == null || (args != null && args.size() > 0) || (indexes != null && indexes.size() > 0)) {
                            return new FunObjectWrapper(owner, args, indexes, context);
                        } else {
                            return new FunObjectWrapper(entry.def, entry.args, null, context);
                        }
                        ...
                        if (nameNode != null) {
                            ...
                            return new FunObjectWrapper(((AliasedDefinition) def).getAliasedDefinition(context), nameNode.getArguments(), nameNode.getIndexes(), context);
                        }
                    }
                    ...
                    if ((cacheability & CACHE_STORABLE) == CACHE_STORABLE) {
                        ...
                        if (data instanceof FunObjectWrapper) {
                            Construction construction = ((FunObjectWrapper) data).getConstruction();
                            if (construction instanceof ResolvedInstance) {
                                ri = (ResolvedInstance) construction;
                            }
                            Definition objDef = ((FunObjectWrapper) data).getDefinition();
                            if (objDef != null) {
                                def = objDef;
                        ...
                        if (def != null) {
                            if (def.isIdentity() && args != null && args.size() == 1) {
                                FunNode node = (FunNode) args.get(0);
