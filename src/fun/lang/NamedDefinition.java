...
public class NamedDefinition extends AnonymousDefinition {
    ...
    public static NameNode createNameNode(String name) {
        ...
        // of FunParser to properly parse the string and create whatever nodes
    ...
    public NamedDefinition(String name, FunNode parent, Definition owner, Type superType, int access, int dur, Object value) {
        super();
    ...
    public Object getChild(NameNode node, ArgumentList args, List<Index> indexes, ArgumentList parentArgs, Context context, boolean generate, boolean trySuper, Object parentObj, Definition resolver) throws Redirection {
        ...
        if (node.getName() == Name.OWNER) {
            ...
            if (generate) {
                ...
                return new FunObjectWrapper(this, args, indexes, context);
            } else {
                Definition aliasedDef = new AliasedDefinition(this, node);
                return aliasedDef.getDefInstance(args, indexes);
            }
        ...
        if (parentObj != null && parentObj instanceof FunObjectWrapper) {
        ...
        if (n > 1 && node.isComplex()) {
            ...
            if (prefixArgs == null) {
                ...
                if (holder != null && holder.def != null && holder.def.getDurability() != DYNAMIC && isDynamic() && !((FunNode) holder.def).isDynamic() && !holder.def.equals(context.getDefiningDef())) {
...
//                    }
                    ...
                    if (paramLists != null && paramLists.size() > 1) {
                        ...
                        for (int i = 0; i < numArgs; i++) {
                            ...
                            if (superArg instanceof Instantiation) {
                                FunNode ref = ((Instantiation) superArg).getReference();
                                if (node.equals(ref)) {
                ...
                if (elementRef == null) {
                    while (def.isReference() && !def.isCollection()) {
                        ...
                        if (childDef != null) {
                            ...
                            FunNode ref = refInstance.getReference();
     ...
     *  can be supported.nitialized.  See the Linker class in fun.lang.SiteLoader.
     */
