...
import funcore.CoreSource;
import fun.runtime.FunVisitor;
...
/**
 * A SiteLoader loads a fun path.  Parsing occurs in a background thread.  This
...
public class SiteLoader {
    ...
    private List<FunSourceLoader> loaders = new ArrayList<FunSourceLoader>();
    protected Core core;
    ...
    /** Load the Fun source code for this site.  The loading occurs in three
     *  steps:
     *  <ol>
     *  <li>If config.fun is present as a resource in the classpath, it is loaded; else all .fun files in the externally specified funpath are loaded.</li>
     *  <li>If there is a definition for "funpath" in a file loaded in the first step,
     *  all .fun files in the path it specifies are loaded.</li>
     *  <li>If the Fun core was not loaded after the first two steps, and the loadCore
     *  property for this site is true, then the core fun files are loaded from the
     *  corepath, if a definition for "corepath" has been loaded, or else as a resource
     *  from the funcore directory in the classpath.</li>
     */
    public void load() {
        String configPath = "/config.fun";
        Class<?> c = getClass();
        URL url = null;
        if (externalUrl != null) {
        ...
        // core.fun and core_platform_java.fun using the loader from this class,
        // which will most often be loading from fun.jar
        if (loadCore) {
    ...
    private void loadFile(File path, String filter, List<FunSourceLoader> loaders, boolean recursive, boolean wait) {
        try {
            ...
            if (path.isDirectory() || isWildCard(path)) {
                ...
                if (load) {
                    log("Loading " + abspath + "...");
                    FunSourceLoader fileLoader = new FunSourceLoader(path, actions);
    ...
    private void loadURL(URL url, List<FunSourceLoader> loaders, boolean wait) {
        try {
            ...
            if (load) {
                ...
                FunSourceLoader urlLoader = new FunSourceLoader(url, actions);
                urlLoader.load(wait);
    ...
    private void loadString(String src, List<FunSourceLoader> loaders, boolean wait) {
        try {
            ...
            FunSourceLoader srcLoader = new FunSourceLoader(reader, actions);
            srcLoader.load(wait);
    ...
    private void link(List<FunSourceLoader> loaders) {
    ...
    public static class Linker extends FunVisitor {
...
//            
    ...
    private class FunSourceLoader implements Runnable {
        ...
        public FunSourceLoader(Object source, int actions) {
            this.source = source;
        ...
        private FunParser getFunParser() throws IOException {
            if (source instanceof Reader) {
                return new FunParser((Reader) source);
            } else {
                return new FunParser(getInputStream());
            }
        }
