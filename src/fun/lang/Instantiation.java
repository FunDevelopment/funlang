...
public class Instantiation extends AbstractConstruction implements ValueGenerator /*, ConstructionGenerator */ {
    ...
    protected void setReference(Object obj) {
        if (obj instanceof FunNode) {
            reference = (FunNode) obj;
        } else {
...
//                    && !((FunNode) holder.nominalDef).isDynamic() 
                        ...
                        //for (int np = 1; np < n; np++) {
                            ...
                            //if (prefixIndexes == null) {
                                ...
                                if (holder != null && holder.nominalDef != null && !holder.nominalDef.equals(context.getDefiningDef())) {
                                    def = holder.nominalDef;
                                    if (holder.nominalDef.getDurability() != Definition.DYNAMIC && !((FunNode) holder.nominalDef).isDynamic() && (args == null || !args.isDynamic())) {
                                        if (holder.data != null && holder.data != UNDEFINED) {
                                            data = holder.data;
                                            // the following breaks a bunch of pent_game_tests for some reason
                                            //if (data instanceof FunObjectWrapper && (n - np) == 1 && generate) {
                                            //    return ((FunObjectWrapper) data).getChildData(restOfName);
                                            //}
                                        }
...
//                            nm = name.getName();
//                            holder = context.getDefHolder(nm, nm, args, indexes, false);
//                            if (holder != null && holder.def != null && holder.def.getDurability() != Definition.DYNAMIC && !((FunNode) holder.def).isDynamic() && !holder.def.equals(context.getDefiningDef())) {
//                                def = holder.def;
//                                args = holder.args;
//                                if (holder.data != null && holder.data != UNDEFINED) {
//                                    data = holder.data;
...
//            }
            ...
            // note: remove the exDef check if you want fun definitions
...
//                }
    ...
    private static Definition lookupDef(NameNode name, List<Index> indexes, FunNode parent, NamedDefinition owner, Definition classDef, Context context, Definition resolver) throws Redirection {
        Definition def = null;
...
//                    }
    ...
    private static Definition lookupSpecialName(NameNode nameNode, FunNode parent, Definition owner, Context context) throws Redirection {
        int numUnpushes = 0;
        try {
            Definition def = null;
            ...
            if (name == Name.SUB) {
                ...
                Definition superdef = site.getDefinition("fun_context");
                Type superType = (superdef != null ? superdef.getType() : null);
                FunContext funContext = new FunContext(site, context);
                def = new ExternalDefinition(Name.HERE, parent, owner, superType, Definition.LOCAL_ACCESS, Definition.DYNAMIC, funContext, null);
             }
