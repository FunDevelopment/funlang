...
public class ExternalDefinition extends ComplexDefinition {
    ...
    public static Definition createForName(Definition owner, ComplexName nameNode, Type superType, int access, int dur, Context context) {
        ...
        if (n == numParts) {
            return new ExternalDefinition(nameNode, (FunNode) owner, owner, superType, access, dur, externalClass, namePart.getArguments());
        }
        
        ExternalDefinition externalDef = new ExternalDefinition(name, (FunNode) owner, owner, superType, access, dur, externalClass, namePart.getArguments());
        if (context == null) {
    ...
    //    this(null, (FunNode) owner, owner, superType, access, dur, object, args);
    //}

    public ExternalDefinition(String internalName, FunNode parent, Definition owner, Type superType, int access, int dur, Object object, ArgumentList args) {
        this(createNameNode(internalName), parent, owner, superType, access, dur, object, args);
    }
        
        
    public ExternalDefinition(NameNode name, FunNode parent, Definition owner, Type superType, int access, int dur, Object object, ArgumentList args) {
        super();
        setAccess(access);
        setDurability(dur);
        setOwner(owner);
...
//   }
    ...
    public List<Dim> getDims() {
        if (dims == null) {
            ...
            } else if (c.isArray() || List.class.isAssignableFrom(c) || FunArray.class.isAssignableFrom(c)) {
...
//               vlog("No field " + name + " in class " + clazz.getName());
    ...
    public int getObjectSize() {
        ...
        } else if (obj instanceof FunArray) {
            return ((FunArray) obj).getSize();
    ...
    static boolean  isCollectionClass(Class<?> clazz) {
    	 return (clazz.isArray() || 
    	         FunArray.class.isAssignableFrom(clazz) ||
    ...
    /** Returns true if the specified class represents a numeric type.  In Fun,
...
class MethodDefinition extends ExternalDefinition {
    ...
    public Object getChild(NameNode node, ArgumentList args, List<Index> indexes, ArgumentList parentArgs, Context argContext, boolean generate, boolean trySuper, Object parentObj, Definition resolver) throws Redirection {
        ...
        if (parentObj instanceof FunObjectWrapper) {
            return ((FunObjectWrapper) parentObj).getChild(node, args, indexes, parentArgs, generate, trySuper, null, resolver);
        } else {
    ...
    public ExternalConstruction initExternalObject(Context context, ArgumentList args) throws Redirection {
        ...
        for (int i = 0; i < numArgs; i++) {
            ...
            // dereference value generators
                ...
                if ((argVal == null || argVal.equals(NullValue.NULL_VALUE)) && arg instanceof FunNode) {
                    Definition argOwner = ((FunNode) arg).getOwner();
                    int numUnpushes = 0;
                    try {
...
//                if (argVal instanceof FunObjectWrapper) {
//                    argVal = ((FunObjectWrapper) argVal).getData();
//                    if (argVal instanceof FunObjectWrapper) {
//                        argVal = null;
//                    }
//                }
            ...
            // dereference collections represented as FunArray objects
            } else if (arg instanceof FunArray) {
                argObjects[i] = ((FunArray) arg).instantiateArray(context);
            ...
            } else if (arg instanceof FunObjectWrapper) {
                argObjects[i] = ((FunObjectWrapper) arg).getData();
