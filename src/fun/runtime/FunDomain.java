/* Fun
 *
 * $Id: FunDomain.java,v 1.32 2015/06/15 13:18:24 sthippo Exp $
 *
 * Copyright (c) 2003-2017 by fundev.org
 *
...
/**
 * This interface extends the fun_domain interface, which correspondes to the fun_domain
 * object defined in core.  It represents a Fun domain, which consists of Fun code loaded
 * under a particular set of restrictions.
 */
public class FunDomain implements fun_domain {
    ...
    private FunDomain mainSite;
    private String domainName;
    private String domainType;
    private fun_processor funProcessor;
    private Map<String, FunDomain> childDomains = null;
    ...
    /** Constructs a new root FunDomain. 
     * 
     *  A FunDomain is capable of loading and compiling Fun source code
    ...
    public FunDomain(String name, fun_processor processor) {
        ...
        mainSite = this;
        funProcessor = processor;
        domainName = name;
        domainType = processor.domain_type();
    }

    /** Constructs a child FunDomain. 
     */
    private FunDomain(FunDomain mainSite, String name, String domainType) {
        if ("core".equals(name)) {
            throw new IllegalArgumentException("The name \"core\" is reserved.");
        ...
        if (mainSite.childDomains == null) {
            mainSite.childDomains = new HashMap<String, FunDomain>();
        }
        mainSite.childDomains.put(name, this);

        this.mainSite = mainSite;
        this.childDomains = mainSite.childDomains;
        this.funProcessor = mainSite.funProcessor;
        this.domainName = name;
        this.domainType = domainType;
        this.core = new Core();
    }

    public FunDomain getChildDomain(String name, String type) {
        FunDomain child = null;
    ...
    public FunDomain createChildDomain(String name, String type) {
        return new FunDomain(this, name, type);
    }
    ...
    private synchronized boolean reload(SiteLoader loader, Core core) {
        ...
        if (!domainName.equals(Name.DEFAULT)) {
            ...
            if (site == null) {
                log("No data for site " + domainName + " in funpath; creating empty site");
                site = new Site(domainName);
                core.addSite(site);
            }
        } else if (defaultSite == null) {
            log("No data for site in funpath.");
        }
    ...
    public Object getPropertyCollectionObject(String name) {
        ...
        try {
            ...
            if (collectionObj instanceof FunArray) {
                collectionObj = ((FunArray) collectionObj).instantiateArray(context);
            } 
    ...
    void globalInit() {
        if (!globallyInitialized) {
            ...
            addExternalObject(this, "this_domain", "fun_domain");
            addExternalObject(funProcessor, "this_processor", "fun_processor");
            addExternalObject(funProcessor, "this_server", "fun_server");
    ...
    public String toString() {
        String str = "Fun Domain " + (domainName != null ? domainName : "(no name)") + '\n';
        if (site != null) {
            str += site.toString();
        } else {
            str += "(empty)";
    ...
    public Object get(String expression) throws Redirection {
        try {
            FunParser parser = new FunParser(new StringReader(expression));
    ...
    public Object get_instance(String expression) throws Redirection {
        try {
            ...
            return new FunObjectWrapper(construction, this);
        } catch (Exception e) {
            System.out.println("Exception getting definition: " + e);
            System.out.println("Expression:\n" + expression);
            return null;
     ...
     *  domain name).
     **/
    public Map<String, FunDomain> domains() {
        return childDomains;
    }
    ...
    /** Creates a fun_context object (represented by the FunContext interface), which can
     *  be used to construct objects.  The fun_context will be able to construct objects in
     *  any of the sites in this domain.
     */
    public fun_context context() {
        return new FunContext(this);
    }
    ...
    public fun_domain child_domain(String name, String type, String src, boolean isUrl) {
        FunDomain child = createChildDomain(name, type);
        if (!child.isLoaded()) {
