...
/**
 * Runtime utility methods accessible to Fun code.
 *
...
public class Utils {
    ...
    public static FunFile getFile(String base, String path) {
        if (base == null) {
            base = ".";
        }
        if (path == null) {
            return new FunFile(base);
        } else {
            return new FunFile(base, path);
        }
    }
    
    public static FunFile getFunFile(FunFile fbase, String base, String path) {
        if (path == null) {
            return (fbase == null ? new FunFile(base) : new FunFile(fbase));
        } else {
            return (fbase == null ? new FunFile(base, path) : new FunFile(fbase, path));
        }
    }    
