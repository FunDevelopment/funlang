...
public class Context {
    ...
    // debugging interface
    private FunDebugger debugger = null;
    ...
    public FunDebugger getDebugger() {
        return debugger;
    }
    
    public void setDebugger(FunDebugger debugger) {
...
//        }    
    ...
    private Object instantiateParameter(DefParameter param, Object arg, NameNode argName) throws Redirection {
        ...
        if (arg instanceof Instantiation) {
            ...
            FunNode argRef = argInstance.getReference();
    ...
    private Object instantiateParameterChild(ComplexName childName, DefParameter param, Object arg, List<Index> indexes) throws Redirection {
        if (arg instanceof Value && !(arg instanceof Instantiation)) {
            ...
            if (val instanceof FunObjectWrapper) {
                arg = val;
            }
        }
        Object data = null;
        ...
        try {
            ...
            if (argDef != null) {
                ...
                if (argDef.isIdentity() && (instance == null || !(instance instanceof ResolvedInstance))) {
                    ...
                    if (holder != null) {
                        if (holder.data instanceof FunObjectWrapper) {
                            FunObjectWrapper wrapper = (FunObjectWrapper) holder.data;
                            data = wrapper.getChildData(childName);
                            if (indexes != null) {
                                data = dereference(data, indexes);
                            }
    ...
    public Object getDescendant(Definition parentDef, ArgumentList args, NameNode name, boolean generate, Object parentObj) throws Redirection {
        ...
        try {
            ...
            if ((args == null || !args.isDynamic()) && !(def instanceof AliasedDefinition)) {
                ...
                if (holder != null && holder.nominalDef != null && holder.nominalDef.getDurability() != Definition.DYNAMIC && !((FunNode) holder.nominalDef).isDynamic()) {
                    def = holder.nominalDef;
                    args = holder.nominalArgs;
                    if (generate && def.isIdentity() && holder.data != null && holder.data instanceof FunObjectWrapper && numNameParts == 1) {
                        FunObjectWrapper obj = (FunObjectWrapper) holder.data;
            ...
            if (!def.isExternal() && (!def.isCollection() || parentObj == null) && !childName.isSpecial()) {
                ...
                while (nextDef.isAliasInContext(this) && !nextDef.isCollection()) {
                    ...
                    if ((args == null || !args.isDynamic()) && aliasIndexes == null) {
                        ...
                        if (holder != null && holder.nominalDef != null && holder.nominalDef.getDurability() != Definition.DYNAMIC && !((FunNode) holder.nominalDef).isDynamic() && (nextDef.equals(holder.nominalDef) || nextDef.equals(holder.def))) {
                            nextDef = holder.nominalDef;
                            args = holder.nominalArgs;
                            if (generate && holder.data != null && holder.data instanceof FunObjectWrapper) {
                                FunObjectWrapper obj = (FunObjectWrapper) holder.data;
                                if (numNameParts == 1) {
                                    try {
                                        unpush();
                    ...
                    if (nominalDef != null && !nominalDef.isCollection() && nominalDef.getDurability() != Definition.DYNAMIC) { 
                        ...
                        if (generate && !dynamicChild) { // && fullNm.equals(childDef.getFullNameInContext(this))) {
                            if (holder.data != null && !holder.data.equals(NullValue.NULL_VALUE)) {
                                if (numNameParts == 1) {
                                    return holder.data;
                                } else if (holder.data instanceof FunObjectWrapper) {
                                    FunObjectWrapper obj = (FunObjectWrapper) holder.data;
            ...
            // if parentObj is a FunObjectWrapper and we are generating data, delegate to the object
            if (generate && childDef.getDurability() != Definition.DYNAMIC && !dynamicChild && numNameParts == 1 && parentObj != null && parentObj instanceof FunObjectWrapper) {
                FunObjectWrapper obj = (FunObjectWrapper) parentObj;
...
//        return dereference(def, args, indexes);
    ...
    public Object dereference(Object data, List<Index> indexes) throws Redirection {
        ...
        if (data instanceof FunArray) {
            data = ((FunArray) data).getArrayObject();
        }
        Iterator<Index> it = indexes.iterator();
    ...
    public Object getParameter(NameNode name, boolean inContainer, Class<?> returnClass) throws Redirection {
        ...
        try {
            ...
            if (checkForChild && argDef != null) {
                ...
                if (arg instanceof PrimitiveValue && FunObjectWrapper.class.equals(((PrimitiveValue) arg).getValueClass())) {
                    FunObjectWrapper wrapper = (FunObjectWrapper) ((Value) arg).getValue();
